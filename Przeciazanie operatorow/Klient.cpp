...
const int _id):
Czlowiek(_imie, _nazwisko, _telefon, _ulica, _miasto, _email)
{}

ostream &operator << (ostream &C, Klient &V)
{
	return C  << " 1 " << " ; " << V.imie << " ; " << V.nazwisko << " ; " << V.telefon << " ; " << V.email << " ; "<< V.ulica << " ; " 
		<< V.miasto << endl;
}
...
{
	string srednik;
	C >> srednik >> V.imie >> srednik >> V.nazwisko >> srednik >> V.telefon >> srednik >> V.email >> srednik >> V.ulica >> srednik 
		>> V.miasto;
	return C;
}
